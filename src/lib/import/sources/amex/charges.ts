...
export function parseNewCharges(
	...
	while (lastAmountValue.isAbove(nextSectionHeader)) {
		...
		const [dateValue, descriptionValue, locationValue, stateValue] = amountValue
			.collectLeft(/\w+/)
			.reverse();
		...
		if (!dateValue) {
			return Result.err(
				ParseStatementError.MissingValue('Missing "Date" value', {
					pageNumber: (dateValue ?? amountValue).pageNumber,
		...
		if (!descriptionValue) {
			return Result.err(
				ParseStatementError.MissingValue('Missing "Description" value', {
					pageNumber: amountValue.pageNumber,
					searchFromText: amountValue.text,
					searchDirection: 'left'
				})
			);
