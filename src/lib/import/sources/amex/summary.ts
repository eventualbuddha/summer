...
export function parseStatementSummary(page: Page): Result<StatementSummary, ParseStatementError> {
	...
	const accountNameValue = accountHolderLabel.findUp(/\w+/, { alignment: 'left', maxGap: 30 });

	if (!accountNameValue) {
		return Result.err(
			ParseStatementError.MissingValue('Missing account name value', {
				pageNumber: page.pageNumber
			})
		);
	}

	const accountNameParts = accountNameValue.collectRight(/\w+/, { maxHorizontalError: 30 });
	...
	const accountName = [accountNameValue, ...accountNameParts]
		.map(({ text }) => text.str.trim())
		.join(' ');
	...
	return Result.ok(
		new StatementSummary(closingDate, account, accountName, accountHolder, ...result.value)
	);
}
...
export class StatementSummary extends StatementMetadata {
	...
	constructor(
		...
		super(closingDate, 'credit', account, accountName);
