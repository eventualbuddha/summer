...
export class StatementTextLocation {
	...
	collectRight(
		finder: Finder,
		options: { maxHorizontalError?: number; maxVerticalError?: number } = {}
	): StatementTextLocation[] {
		return this.#pageLocation
			.collectRight(finder, options)
			.map(
				(pageLocation) =>
					new StatementTextLocation(
						this.#pages,
	...
	[Symbol.for('nodejs.util.inspect.custom')]() {
		return `StatementTextLocation { text: ${this.text.str}, pageNumber: ${this.#pageNumber}, x=${this.text.x}, y=${this.text.y}, right=${this.text.right} }`;
	}
}
...
export class PageTextLocation {
	...
	findRight(
		...
		const predicate = predicateForFinder(finder);
		const { right, centerX, centerY } = this.#text;
		...
		const scoredLocation = this.#navigator.findScored((text) => {
			...
			const verticalError = Math.abs(text.centerY - centerY);
			if (verticalError > maxVerticalError) {
				return Infinity;
			}

			const endsRight = text.right > right;
			const centerIsRight = text.centerX > centerX;

			if (!endsRight && !centerIsRight) {
				return Infinity;
			}

			const horizontalError = Math.abs(text.left - right);
			if (horizontalError > maxHorizontalError) {
				return Infinity;
			}

			return horizontalError ** (right <= text.left ? 0.5 : 2) + verticalError ** 2;
		});
	...
	findLeft(
		...
		const predicate = predicateForFinder(finder);
		const { centerX, centerY, left } = this.#text;
		...
		const scoredLocation = this.#navigator.findScored((text) => {
			...
			const startsLeft = text.left < left;
			const centerIsLeft = text.centerX < centerX;

			if (!startsLeft && !centerIsLeft) {
				return Infinity;
			}

			const horizontalError = Math.abs(text.right - left);
			if (horizontalError > maxHorizontalError) {
				return Infinity;
			}

			return horizontalError ** (left >= text.right ? 0.5 : 2) + verticalError ** 2;
		});
	...
	collectRight(
		...
		let nextRight: PageTextLocation | undefined = this;
		const locations: PageTextLocation[] = [];
		while (true) {
			nextRight = nextRight?.findRight(finder, options);
			if (!nextRight) break;
			locations.push(nextRight);
		}
		return locations;
	}
	...
	collectLeft(
		...
		// eslint-disable-next-line @typescript-eslint/no-this-alias
		let nextLeft: PageTextLocation | undefined = this;
		const locations: PageTextLocation[] = [];
		while (true) {
			nextLeft = nextLeft?.findLeft(finder, options);
			if (!nextLeft) break;
			locations.push(nextLeft);
		}
		return locations;
	}
